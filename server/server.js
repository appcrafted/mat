...
app.post('/getdetailByEmail', (req, res) => {
  var email = req.body.email;

  Licence.findOne({email: email}, {}, { sort: { '_id' : -1 } }).then((doc) => {
    if (!doc) {
...
app.post('/verifyKey', (req, res) => {
  ...
  Licence.findOne({key}).then((doc) => {
    ...
    Licence.findOneAndUpdate({key}, {$set: {days,reg,ad,ed,active,email,name,password}}, {new: true}).then((doc) => {
    if (!doc) {
      return res.status(404).send();
    }
...
app.post('/createKeys', authenticate, (req, res) => {
  ...
  for (var i = 0; i < number; i++) {
    ...
    var licence = new Licence({
      key : "mat-" + shortid.generate().toLowerCase(),
      days : req.body.days,
...
app.post('/updateKey', authenticate, (req, res) => {
  ...
  var key = req.body.key.toLowerCase();
  var body = _.pick(req.body, ['key', 'ad','ed', 'email','active','reg','year']);
